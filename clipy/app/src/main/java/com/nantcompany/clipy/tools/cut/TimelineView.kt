...
class TimelineView : View {
    ...
    private fun initAttr(attrs: AttributeSet) {
        ...
        try {
            ...
            // Optional: Handle case where resources might be missing
        } finally {
            ta.recycle()
        }
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                when {
                    abs(x - width * progressLeft) < thumbWidth * 4 -> { // Increased hit area
                        pressedBtn = 0
                    }
                    abs(x - (width * currentProgress)) < thumbWidth * 4 -> {
                        seeking = true
                    ...
                    abs(x - width * progressRight) < thumbWidth * 4 -> {
                        pressedBtn = 2
                    }
            ...
            MotionEvent.ACTION_MOVE -> if (pressedBtn >= 0) {
                val newProgress = (x / measuredWidth).coerceIn(0f, 1f)
    ...
    fun processBitmap(frameNum: Int): Bitmap? {
        try {
            ...
            if (bitmap != null) {
                val result = Bitmap.createBitmap(frameWidth, frameHeight, bitmap.config ?: Bitmap.Config.ARGB_8888)
    ...
    private fun loadFrame(framePosition: Int) {
        if (framePosition == 0) {
            if (calcHeight <= 0) {
                postDelayed({ loadFrame(0) }, 100) // Wait for layout
                return
            }
            frameHeight = calcHeight
            frameWidth = (frameHeight * frameDimentionRatio).toInt()
            frameToLoad = (calcWidth / frameWidth).coerceAtLeast(1) + 1
            frameOffset = if (frameToLoad > 0) ((totalDuration / frameToLoad).toInt()) else 0
        }
        ...
        object : AsyncTask<Int, Void, Bitmap?>() {
            ...
            override fun onPostExecute(result: Bitmap?) {
                ...
                if (result != null)
                    frames.add(result)
                if (frameNum < frameToLoad - 1) {
                    loadFrame(framePosition + 1)
                }
                invalidate()
            }
        }
        task.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, framePosition)
    }
    ...
    fun setVideoUri(path: Uri) {
        try {
            ...
            totalDuration = duration?.toLong() ?: 0
            loadFrame(0)
            invalidate()
        } catch (ex: Exception) {
            ex.printStackTrace()
