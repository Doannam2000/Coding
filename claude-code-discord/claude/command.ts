...
const DONE_TOKEN_REGEX = /\bDONE\b/i;
const SMART_COMPLETE_REGEX = /\b(completed|complete|finished|all done|task done|implemented successfully|done successfully)\b/i;
const AUTO_DONE_MAX_CYCLES = 12;
const AUTO_DONE_MAX_MS = 25 * 60 * 1000;

function hasDoneToken(text?: string): boolean {
  if (!text) return false;
  return DONE_TOKEN_REGEX.test(text) || SMART_COMPLETE_REGEX.test(text);
}
...
export function createClaudeHandlers(deps: ClaudeHandlerDeps) {
  ...
  return {
    ...
    async onClaude(ctx: any, prompt: string, channelId: string, explicitSessionId?: string): Promise<ClaudeResponse> {
      ...
      const queryOptions = deps.getQueryOptions?.();
      const startedAt = Date.now();
      let cycle = 0;
      let currentPrompt = prompt;
      let currentSessionId = activeSessionId;
      let latestResult: ClaudeResponse | undefined;

      while (cycle < AUTO_DONE_MAX_CYCLES && (Date.now() - startedAt) < AUTO_DONE_MAX_MS) {
        cycle++;
        ...
        if (currentSessionId) {
          deps.setSessionForChannel(channelId, currentSessionId);
          deps.setClaudeSessionId(currentSessionId);
        }

        if (hasDoneToken(result.response)) {
          break;
        }

        currentPrompt = "Continue from your exact previous state. Do not restate progress. Execute next steps immediately and reply DONE only when fully finished.";
      }

      deps.setClaudeController(null);

      return latestResult || { response: "No response received" };
    },
    ...
    async onClaudeThread(ctx: any, prompt: string, threadName?: string): Promise<ClaudeResponse> {
      ...
      const finalResult = latestResult || { response: "No response received" };

      // Map the thread channel → session so /claude inside the thread auto-continues
      if (threadSessionKey && finalResult.sessionId && deps.sessionThreads) {
        deps.sessionThreads.updateSessionId(threadSessionKey, finalResult.sessionId);
      }
      if (threadChannelId && finalResult.sessionId) {
        deps.setSessionForChannel(threadChannelId, finalResult.sessionId);
      }
    ...
    async onContinue(ctx: any, prompt?: string): Promise<ClaudeResponse> {
      ...
      const startedAt = Date.now();
      let cycle = 0;
      let currentPrompt = actualPrompt;
      let currentSessionId: string | undefined = undefined;
